...
$(document).ready(function() {
  ...
  if (filename) {
    ...
    .done(function (data) {
      console.log("SUCCESS");
      var status_style = (data.status == 'failed') ? '<i class="fa fa-times-circle" style="color:#A00;font-size:18px"></i> Failed' : '<i class="fa fa-check-circle" style="color:#0A0;font-size:18px"></i> Passed';
      ...
      // Error messages
      if (data.error) {
        var report = '<h5 class="mt-4"><i class="fa fa-times-circle" style="color:#A00"></i> Error report</h5>'+
      ...
      // Warning messages
      if (data.warning) {
        var report = '<h5 class="mt-4"><i class="fa fa-exclamation-triangle" style="color:orange"></i> Warning report</h5>'+
                     '<table class="table table-bordered" style="width:auto"><thead class="thead-light">'+
    ...
    .fail(function (xhRequest, ErrorText, thrownError) {
      var status_html = '<div><b>File validation:</b> <i class="fa fa-times-circle-o" style="color:#A00"></i> Failed</div>';
      $('#check_status').html(status_html);
  ...
  function report_items_2_html(reports_list) {
    ...
    $.each(reports_list, function(index, report_item){
      ...
      // Leading space
      message = message.replace(/\"<b>\s+/g, "\"<b><span style=\"color:#A00\">_</span>");
      // Trailing space
      message = message.replace(/\s+<\/b>\"/g, "<span style=\"color:#A00\">_</span></b>\"");
      // Column highlighting
