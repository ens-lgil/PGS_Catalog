...
//   '<span style="font-size:16px">&#x25CF;</span>', // Circle
//   '<span style="font-size:12px">&#x25B2;</span>', // Triangle
//   '<span style="font-size:16px">&#x2666;</span>', // Diamond
//   '&#x25A0;',                                     // Square
//   '<span style="font-size:13px">&#9733;</span>',  // Star
//   '&#x271A;',                                     // Cross
//   // Second set with same shapes but outlined (i.e stroke) instead of filled
//   '<span style="font-size:16px">&#x25CB;</span>', // Circle outline
//   '<span style="font-size:12px">&#x25B3;</span>', // Triangle outline
//   '<span style="font-size:12px">&#x2662;</span>', // Diamond outline
//   '&#x25A1;',                                     // Square outline
//   '<span style="font-size:13px">&#9734;</span>',  // Star outline
//   '&#x2719;'                                      // Cross outline
// ];
...
class PGSBenchmark {
  ...
  addData() {
    ...
    chart_content.selectAll('rect')
      ...
      .attr("transform",function(d) { return "translate(" + obj.x0(d.pgs) + ",0)"; })
      .each(function(d,i){
        //var colour = obj.z(d.cohortAncestry);
  ...
  addTooltip(elem, data) {
    ...
    //var title = '<div class="tooltip_content"><div class="tooltip_section" style="border:5px solid '+color+'"><b>'+data.anc + '</b> ('+data.cohortAncestry.split(sep)[0]+')</div>';
...
$( document ).ready(function() {
  ...
  $("#exportPNG").on("click", function() {
    ...
    svgString2Image( svgString, 2*svg_width, 2*svg_height, 'png');//, save ); // passes Blob and filesize String to the callback
...
function fill_cohort_form(data) {
  ...
  for (var i=0; i<cohort_length;i++) {
    ...
    html_cb += '<div>'+
               ...
               '</div>';
    // html_cb += '<div class="custom-control custom-switch">'+
    //            '  <input type="checkbox" class="custom-control-input benchmark_cohort_cb" data-default="true" checked value="'+cohort+'" id="'+id+'">'+
    //            '  <label class="custom-control-label mb-0" for="'+id+'"> '+cohort+' (<span class="'+cohort_shape+'"></span>)</label>'+
    //            '</div>';
...
function fill_ancestry_form(data, cohorts) {
  ...
  for (var l=0; l<ancestry_list.length;l++) {
    ...
    html_cb += '<div>'+
               ...
               '</div>';
    // html_cb += '<div class="custom-control custom-switch">'+
    //            '  <input type="checkbox" class="custom-control-input benchmark_ancestry_cb" data-default="true"'+is_checked+is_disabled+' value="'+ancestry+'" id="'+id+'">'+
    //            '  <label class="custom-control-label mb-0" for="'+id+'"> '+ancestry+' (<span class="'+cb_class+'"></span>)</label>'+
    //            '</div>';
  }
